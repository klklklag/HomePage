...
import { addIdIndexToTableData, COLORS, decrypt, refreshPage } from '@resources';
import { adminAPI, workerAPI, WorkerData, WorkerEmail, WorkerLogListItem, WorkerPartnerData } from '@api';
import {
  HoldPaymentModal,
  InputSelectBank,
...
export const getServerSideProps = async ({ query }: GetServerSidePropsContext) => {
  ...
  const [workerResult, workerPartnerResult, workerLogResult] = await Promise.all([
    workerAPI.readWorkerData({ workerEmail }),
    workerAPI.readWorkerPartnerData({ workerEmail }),
    adminAPI.readWorkerThanksPayAll({ workerEmail }),
  ]);

  if (
    'errors' in workerResult ||
    'errors' in workerPartnerResult ||
    'errors' in workerLogResult
  ) return { props: { data: {} }};
  return {
    props: {
      data: {
        ...workerResult,
        ...workerPartnerResult,
        workerLogList: addIdIndexToTableData(workerLogResult.workerThanksPayListAll || []),
        workerEmail,
      }
    }
  };
...
const Component = ({ data }: { data: Partial<WorkerData & WorkerEmail & WorkerPartnerData & { workerLogList: WorkerLogListItem[]; }>; }) => {
  const { query } = useRouter();
  const [visible, show, hide] = useModal();
  const [holdVisible, holdShow, holdHide] = useModal();

  const relicenseWorker = useCallback(async () => {
    const workerEmail = decrypt(String(query.workerId));
    const result = await adminAPI.updateWorkerState({ workerEmail, isBlack: "0" });
    if ('errors' in result) {
  ...
  return (
    <PartnerScreen>
      <MenuBar />
      <Body>
        <WorkerHeader workerName={data?.bankAccountOwner} salaryDay={data?.partnerSalaryDay} blackState={data?._isBlackWorker} isEmployee={data.isEmployee} />
        ...
        <TitleRow>
          ...
          {
            data._isBlackWorker ?
              <RelicenseThanksPayButton style="margin: 0px 0px 0px auto;" onClick={relicenseWorker} /> :
        ...
        <WorkerMonthlyHistoryTable data={data.workerLogList} />
      </Body>
