...
import { BottomButton, GoBackButton, Modal } from '@components';
import { COLORS, IMAGES } from '@resources';
import { adminAPI } from '@api';
import { ModalInstanceProps } from './base';

const _SendEmailModal = ({
  email,
  ...props
}: {
  email: string;
  ...
  const sendAuthEmail = useCallback(async () => {
    ...
    const result = await adminAPI.sendEmailAuth({ email: email });
    if ('errors' in result) {
    ...
    <Modal {...props} type="plain">
      <Root>
        ...
        <ButtonRow>
          ...
          <BottomButton type="button" text="확인" style={confirmButtonStyle} onClick={props.closeFunc} />
        </ButtonRow>
      </Root>
    </Modal>
  );
...
export const SendEmailModal = _SendEmailModal;